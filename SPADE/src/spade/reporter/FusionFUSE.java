...
public class FusionFUSE extends AbstractReporter {
    ...
    public void receivefile(String remotePath, String remoteIP, String localPath) {
    	...
    	File remoteFileVertex = createFileVertex(remotePath);
    	remoteFileVertex.addAnnotation("hostaddress", remoteIP);
    	File localFileVertex = createFileVertex(localPath);
    	putVertex(remoteFileVertex);
    	putVertex(localFileVertex);
    	
    	// cshou debug
    	fsLogger.info("Putting remote File vertex:\n" + remoteFileVertex);
    	fsLogger.info("Putting local File vertex:\n" + localFileVertex);
    	
    	WasTransferredFrom edge = new WasTransferredFrom(localFileVertex, remoteFileVertex);
    	putEdge(edge);
    	
    	// cshou debug
    	fsLogger.info("Putting WasTransferredFrom egde:\n" + edge);
    }
